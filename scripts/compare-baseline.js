...
async function waitForServer(config) {
  ...
  console.log('❌ Dev server not responding. Please start it with: npm run dev');
  return false;
}
...
function openURL(url, zoom = 100) {
  ...
  try {
    ...
    if (zoom !== 100) {
      console.log(`🔍 Opened at ${zoom}% zoom (you may need to adjust manually)`);
    }
  } catch (error) {
...
async function main() {
  ...
  if (!fs.existsSync(BASELINE_PATH)) {
    ...
    console.log('💡 Make sure the baseline file exists in src/tmp/generated.html');
    process.exit(1);
  }
